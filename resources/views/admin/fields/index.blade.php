...
@section('content')
<div class="bg-slate-800 p-4 rounded-2xl border border-slate-700 mb-6 flex gap-4">
    <a href="{{ route('admin.fields.index') }}" class="px-3 py-1.5 rounded-lg text-xs font-bold {{ !request('status') ? 'bg-indigo-600 text-white' : 'bg-slate-900 text-slate-400' }}">Tất cả sân</a>
    <a href="{{ route('admin.fields.index', ['status' => 'pending']) }}" class="px-3 py-1.5 rounded-lg text-xs font-bold {{ request('status') == 'pending' ? 'bg-amber-500 text-slate-950' : 'bg-slate-900 text-slate-400' }}">Chờ duyệt</a>
    <a href="{{ route('admin.fields.index', ['status' => 'approved']) }}" class="px-3 py-1.5 rounded-lg text-xs font-bold {{ request('status') == 'approved' ? 'bg-emerald-600 text-white' : 'bg-slate-900 text-slate-400' }}">Đã duyệt</a>
</div>
            ...
            @foreach($fields as $f)
                <tr class="hover:bg-slate-750">
                    <td class="px-6 py-4 font-bold text-white">
                        {{ $f->name }}
                        <span class="block text-xs font-normal text-slate-400">{{ $f->address }}</span>
                    </td>
                    <td class="px-6 py-4">{{ $f->owner->name }}</td>
                    <td class="px-6 py-4 text-xs font-semibold text-emerald-400">{{ $f->fieldType->name }}</td>
                    <td class="px-6 py-4 font-bold text-white">{{ number_format($f->price_per_hour) }}đ</td>
                    <td class="px-6 py-4">
