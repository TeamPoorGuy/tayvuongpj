...
export function FieldDetailPage() {
  ...
  const booking = useMutation({
    mutationFn: async () => api.post('/customer/bookings', { sports_field_id: field.data?.id, time_slot_id: slotId, booking_date: date, notes }),
    onSuccess: () => { setSlotId(null); void queryClient.invalidateQueries({ queryKey: ['slots'] }) },
  })

  useReveal([field.data])
  ...
  return (
    <div className="shell page-section detail-page">
      ...
      <div className="detail-grid">
        <div>
          <section className="content-card reveal"><h2>Thông tin sân</h2><p>{item.description || 'Chủ sân chưa cập nhật mô tả chi tiết.'}</p><div className="feature-list"><span><ShieldCheck /> Sân đã kiểm duyệt</span><span><Clock3 /> Khung giờ 90 phút</span><span><Check /> Xác nhận bởi chủ sân</span></div></section>
          <section className="content-card reveal"><h2>Đánh giá gần đây</h2>{item.reviews?.filter((review) => review.is_visible).map((review) => <article className="review" key={review.id}><div><strong>{review.user?.name}</strong><span>{'★'.repeat(review.rating)}</span></div><p>{review.comment}</p></article>)}</section>
        </div>
