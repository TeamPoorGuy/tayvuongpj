...
export function CustomerBookingsPage() {
  ...
  const [activeReviewing, setActiveReviewing] = useState<Booking | null>(null)
  const currentReview = reviewing ?? activeReviewing

  const openReview = (booking: Booking) => {
    setReviewing(booking)
    setActiveReviewing(booking)
  }

  const bookings = useQuery({ queryKey: ['customer', 'bookings'], queryFn: async () => (await api.get<Paginated<Booking>>('/customer/bookings')).data })
  const cancel = useMutation({ mutationFn: async (booking: Booking) => api.patch(`/customer/bookings/${booking.id}/cancel`), onSuccess: () => client.invalidateQueries({ queryKey: ['customer', 'bookings'] }) })
  const review = useMutation({ mutationFn: async (payload: { booking_id: number; rating: number; comment: string }) => api.post('/customer/reviews', payload), onSuccess: () => { setReviewing(null); void client.invalidateQueries({ queryKey: ['customer', 'bookings'] }) } })
  ...
  return <><PageTitle icon={<CalendarDays />} kicker="Lịch chơi" title="Các lượt đặt của tôi" copy="Theo dõi xác nhận, hủy lịch và chia sẻ đánh giá sau trận." />
    ...
    <div className="booking-list">{bookings.data?.data.map((booking, index) => <article className="booking-item reveal" style={{ '--i': index } as React.CSSProperties} key={booking.id}><div className="booking-item__image">{booking.field?.primary_image && <img src={booking.field.primary_image} alt="" />}</div><div className="booking-item__body"><div className="booking-item__top"><div><small>#BK-{String(booking.id).padStart(5, '0')}</small><h3>{booking.field?.name}</h3></div><StatusBadge status={booking.status} /></div><div className="booking-meta"><span><CalendarDays />{new Date(booking.booking_date).toLocaleDateString('vi-VN')}</span><span><Clock3 />{booking.time_slot?.start_time} – {booking.time_slot?.end_time}</span><span><MapPin />{booking.field?.address}</span></div><div className="booking-item__footer"><strong>{currency.format(booking.total_price)}đ</strong><div>{booking.can_be_cancelled && <button className="button button--danger" onClick={() => cancel.mutate(booking)}>Hủy lịch</button>}{booking.status === 'completed' && !booking.review && <button className="button button--brand" onClick={() => openReview(booking)}><Star size={15} /> Đánh giá</button>}</div></div></div></article>)}</div>
    {!bookings.isPending && !bookings.data?.data.length && <Empty text="Bạn chưa có lịch đặt sân nào." />}
    <Modal isOpen={!!reviewing} onClose={() => setReviewing(null)} asForm onSubmit={submitReview}>
      <h2>Đánh giá {currentReview?.field?.name}</h2>
      <label><span>Số sao</span><select name="rating" defaultValue="5">{[5,4,3,2,1].map((n) => <option value={n} key={n}>{n} sao</option>)}</select></label>
      <label><span>Nội dung</span><textarea name="comment" required /></label>
      {review.isError && <div className="notice notice--error">{errorMessage(review.error)}</div>}
      <div className="modal__actions">
        <button type="button" className="button button--ghost" onClick={() => setReviewing(null)}>Đóng</button>
        <button className="button button--brand">Gửi đánh giá</button>
      </div>
    </Modal>
  </>
}
...
export function CustomerProfilePage() {
  ...
  const update = useMutation({ mutationFn: async (payload: FormData) => api.post('/customer/profile', withMethodOverride(payload, 'PUT')), onSuccess: () => { setMessage('Đã lưu hồ sơ.'); void client.invalidateQueries({ queryKey: ['auth', 'me'] }) } })
  const password = useMutation({ mutationFn: async (payload: Record<string, string>) => api.put('/customer/profile/password', payload), onSuccess: () => setMessage('Đã đổi mật khẩu.') })
